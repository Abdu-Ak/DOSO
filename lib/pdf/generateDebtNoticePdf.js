...
export const generateDebtNoticePdf = async (record) => {
  ...
  // 3. Witness 1 Information
  y = sectionHeader(doc, "Witness 1 Information", y, W, margin);
  autoTable(doc, {
    startY: y,
    margin: { left: margin, right: margin },
    body: [
      ...
      ["Email", val(record.witness1?.email)],
  ...
  // 4. Witness 2 Information
  y = sectionHeader(doc, "Witness 2 Information", y, W, margin);
  autoTable(doc, {
    startY: y,
    margin: { left: margin, right: margin },
    body: [
      ...
      ["Email", val(record.witness2?.email)],
  ...
  doc.save(
    `Debt_Notice_${val(record.requester?.userId)}_${val(record._id)}.pdf`,
  );
};
