  ...
  await transporter.sendMail({
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 12px;">
        ...
        <p style="text-align: center;">Use this to go to: <a href="${process.env.NEXTAUTH_URL}/login" style="color: #1a1a2e; font-weight: bold; text-decoration: underline;">Login Page</a></p>
...
export async function sendWelfareApprovalEmail(
  email,
  name,
  description,
  amount,
  ...
  await transporter.sendMail({
    from,
    to: email,
    subject: `Welfare Fund Approval`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 12px;">
        <div style="text-align: center; margin-bottom: 20px;">
          <img src="${logoUrl}" alt="DOSO Logo" style="width: 100px; height: auto;">
        </div>
        <h2 style="color: #1a1a2e; text-align: center;">Welfare Fund Approved!</h2>
        <p>Hello ${name},</p>
        <p>Your Welfare Fund submission for <strong>${description}</strong> has been approved.</p>
        <div style="background: #f4f4f8; padding: 16px; border-radius: 8px; margin: 16px 0;">
          <p style="margin: 4px 0;"><strong>Amount:</strong> ₹${amount}</p>
          <p style="margin: 4px 0;"><strong>Receipt Number:</strong> ${receiptNumber}</p>
        </div>
...
export async function sendWelfareRejectionEmail(
  email,
  name,
  description,
  reason,
  ...
  await transporter.sendMail({
    from,
    to: email,
    subject: `Welfare Fund Submission Update`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 12px;">
        <div style="text-align: center; margin-bottom: 20px;">
          <img src="${logoUrl}" alt="DOSO Logo" style="width: 100px; height: auto;">
        </div>
        <h2 style="color: #1a1a2e; text-align: center;">Welfare Fund Submission Update</h2>
        <p>Hello ${name},</p>
        <p>We regret to inform you that your Welfare Fund submission for <strong>${description}</strong> has not been accepted.</p>
        <div style="background: #fff3f3; padding: 16px; border-radius: 8px; margin: 16px 0; border-left: 4px solid #e74c3c;">
          <strong>Reason:</strong> ${reason}
        </div>
        <p>Please contact the administration or resubmit with the correct details if necessary.</p>
...
export async function sendDebtWitnessNotification(
  email,
  name,
  requesterName,
  portalLink,
  ...
  await transporter.sendMail({
    from,
    to: email,
    subject: `Action Required: Debt Request Witness`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 12px;">
        <div style="text-align: center; margin-bottom: 20px;">
          <img src="${logoUrl}" alt="DOSO Logo" style="width: 100px; height: auto;">
        </div>
        <h2 style="color: #1a1a2e; text-align: center;">Witness Request</h2>
        <p>Hello ${name},</p>
        <p>Alumni <strong>${requesterName}</strong> has requested you as a witness for a debt application.</p>
        <div style="text-align: center; margin: 32px 0;">
          <a href="${portalLink}" style="background-color: #1a1a2e; color: white; padding: 14px 28px; text-decoration: none; border-radius: 8px; font-weight: bold; display: inline-block;">View Request</a>
        </div>
...
export async function sendDebtWitnessRespondNotification(
  email,
  name,
  witnessName,
  status,
  ...
  await transporter.sendMail({
    from,
    to: email,
    subject: `Debt Request Witness Update`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 12px;">
        <div style="text-align: center; margin-bottom: 20px;">
          <img src="${logoUrl}" alt="DOSO Logo" style="width: 100px; height: auto;">
        </div>
        <h2 style="color: #1a1a2e; text-align: center;">Witness Response</h2>
        <p>Hello ${name},</p>
        <p>Witness <strong>${witnessName}</strong> has <strong>${status}</strong> your debt request.</p>
        ${!isApproved && reason ? `<div style="background: #fff3f3; padding: 16px; border-radius: 8px; margin: 16px 0; border-left: 4px solid #e74c3c;"><strong>Reason:</strong> ${reason}</div>` : ""}
        <p>You can track the progress of your request in the Alumni portal.</p>
      </div>
    `,
  });
}

export async function sendDebtAdminRespondNotification(
  email,
  name,
  status,
  receiptNo,
  ...
  await transporter.sendMail({
    from,
    to: email,
    subject: `Debt Request Final Update`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 12px;">
        <div style="text-align: center; margin-bottom: 20px;">
          <img src="${logoUrl}" alt="DOSO Logo" style="width: 100px; height: auto;">
        </div>
        <h2 style="color: #1a1a2e; text-align: center;">Admin Decision</h2>
        <p>Hello ${name},</p>
        <p>Your debt request has been <strong>${status}</strong> by the administration.</p>
        ${isApproved && receiptNo ? `<div style="background: #f4f4f8; padding: 16px; border-radius: 8px; margin: 16px 0;"><strong>Receipt Number:</strong> ${receiptNo}</div>` : ""}
        ${!isApproved && reason ? `<div style="background: #fff3f3; padding: 16px; border-radius: 8px; margin: 16px 0; border-left: 4px solid #e74c3c;"><strong>Reason:</strong> ${reason}</div>` : ""}
        <p>Thank you.</p>
      </div>
    `,
