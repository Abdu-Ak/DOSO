...
import { Box, Search, CheckCircle2, CheckCircle } from "lucide-react";
import { useDebounce } from "@/lib/hooks";

export default function PublicSundookPage() {
  ...
  const {
    ...
    resolver: zodResolver(publicCreateSundookSchema),
    defaultValues: {
      alumni: "",
      box_number: "",
      amount: "",
  ...
  const { data: alumniData, isLoading: isAlumniLoading } = useQuery({
    queryKey: ["public-alumni-search", debouncedSearch],
    queryFn: async () => {
      const response = await axios.get("/api/users", {
        params: {
          ...
          limit: 20,
          status: "Active", // Only show active alumni
        },
      });
  ...
  const mutation = useMutation({
    mutationFn: async (data) => {
      const response = await axios.post("/api/register/sundook", data);
      return response.data;
    },
    onSuccess: () => {
      addToast({
        title: "Success",
        description: "Your Sundook record has been submitted for approval.",
        color: "success",
      });
      setSubmitted(true);
      reset();
        ...
        <Card className="max-w-md w-full">
          <CardBody className="p-8 text-center space-y-4">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Your Sundook record has been submitted and is currently pending
        ...
        <div className="flex flex-col items-center text-center space-y-4">
          ...
          <div className="space-y-2">
            <h1 className="text-2xl sm:text-3xl font-bold text-slate-800 dark:text-white">
              Sundook Registration
            </h1>
            <p className="text-sm sm:text-base text-slate-500 dark:text-slate-400 max-w-md mx-auto">
              Submit your alumni sundook contribution details. Your application
