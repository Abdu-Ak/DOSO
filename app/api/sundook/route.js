import { NextResponse } from "next/server";
import dbConnect from "@/lib/mongodb";
import User from "@/models/User";
import Sundook from "@/models/Sundook";
import { getServerSession } from "next-auth";
import { authOptions } from "@/app/api/auth/[...nextauth]/route";
import {
  createSundookSchema,
  adminCreateSundookSchema,
} from "@/lib/validations/sundook.validation";

export async function GET(request) {
  try {
    await dbConnect();
    const session = await getServerSession(authOptions);

    if (!session) {
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }

    const { searchParams } = new URL(request.url);
    const page = parseInt(searchParams.get("page")) || 1;
    const limit = parseInt(searchParams.get("limit")) || 10;
    const skip = (page - 1) * limit;

    let query = {};

    if (session.user.role === "admin" || session.user.role === "super_admin") {
      const status = searchParams.get("status");
      const alumniId = searchParams.get("alumniId");
      const year = searchParams.get("year");
      const search = searchParams.get("search");

      if (status) query.status = status;
      if (alumniId) query.alumni = alumniId;
      if (year) query.year = parseInt(year);

      if (search) {
        const matchingUsers = await User.find({
          $or: [
            { name: { $regex: search, $options: "i" } },
            { userId: { $regex: search, $options: "i" } },
          ],
        }).select("_id");
        query.alumni = { $in: matchingUsers.map((u) => u._id) };
      }
    } else {
      // Alumni can only see their own records
      query.alumni = session.user.id;
    }

    const total = await Sundook.countDocuments(query);
    const records = await Sundook.find(query)
      .populate("alumni", "name email userId phone image")
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);

    return NextResponse.json({
      records,
      total,
      pages: Math.ceil(total / limit),
      currentPage: page,
    });
  } catch (error) {
    console.error("GET Sundook Error:", error);
    return NextResponse.json(
      { error: "Failed to fetch records" },
      { status: 500 },
    );
  }
}

export async function POST(request) {
  try {
    await dbConnect();
    const session = await getServerSession(authOptions);

    if (!session) {
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }

    const body = await request.json();
    const currentYear = new Date().getFullYear();

    if (session.user.role === "admin" || session.user.role === "super_admin") {
      // Admin creation
      const validatedData = adminCreateSundookSchema.parse(body);

      const newRecord = await Sundook.create({
        ...validatedData,
        status: "approved", // Admin created entries are auto-approved
      });

      return NextResponse.json(newRecord, { status: 201 });
    } else {
      // Alumni submission
      const validatedData = createSundookSchema.parse(body);

      // Check for existing submission in the current year
      const existingRecord = await Sundook.findOne({
        alumni: session.user.id,
        year: currentYear,
      });

      if (existingRecord) {
        return NextResponse.json(
          {
            error: "You have already submitted a Sundook record for this year.",
          },
          { status: 400 },
        );
      }

      const newRecord = await Sundook.create({
        ...validatedData,
        alumni: session.user.id,
        year: currentYear,
        status: "pending",
      });

      return NextResponse.json(newRecord, { status: 201 });
    }
  } catch (error) {
    console.error("POST Sundook Error:", error);
    if (error.name === "ZodError") {
      return NextResponse.json(
        { error: error.errors[0].message },
        { status: 400 },
      );
    }
    return NextResponse.json(
      { error: "Internal Server Error" },
      { status: 500 },
    );
  }
}
